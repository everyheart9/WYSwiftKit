...
public extension NSMutableAttributedString {
    /// 属性字符串拼接字符串
    func wy_appendString(_ str: String, _ attrs: Dictionary<NSAttributedString.Key, Any> = [:]) -> Self {
        let att = NSAttributedString.init(string: str, attributes: attrs)
        return self.wy_appendAttributedString(att)
    }
    
    /// 属性字符串拼接属性字符串
    func wy_appendAttributedString(_ attrsStr: NSAttributedString) -> Self {
        self.append(attrsStr)
        return self
    }
    
    /// 属性字符串根据某个字符串修改属性
    func wy_modifyAttributedString(_ str: String, _ attrs: Dictionary<NSAttributedString.Key, Any> = [:]) -> Self {
    ...
    /// 给某个属性字符串增加属性
    func wy_addAttributed(_ attrs: Dictionary<NSAttributedString.Key, Any> = [:], range: NSRange) -> Self {
        self.addAttributes(attrs, range: range)
        return self
    }

    /// 通过图片和字体设置图片
    func wy_addImageAttributedAndFont(image: UIImage, font: UIFont) -> Self {
        ...
        return self.wy_appendAttributedString(imageAtt)
    }
